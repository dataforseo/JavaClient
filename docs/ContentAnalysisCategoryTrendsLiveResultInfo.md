...
**rank** | **Integer** | <em>rank of all URLs citing the <code>keyword</code></em><br>normalized sum of ranks of all URLs citing the target <code>keyword</code> for the given date |[optional]|
**topDomains** | **List<TopDomainInfo>** | <em>top domains citing the target keyword</em><br>contains objects with top domains citing the target category and citation count per each domain |[optional]|
...
**connotationTypes** | **Map<String, Long>** | <em>connotation types</em><br>contains types of sentiments (sentiment polarity) related to the category citation and citation count per each sentiment type<br>possible connotation types: <code>'positive'</code>, <code>'negative'</code>, <code>'neutral'</code> |[optional]|
