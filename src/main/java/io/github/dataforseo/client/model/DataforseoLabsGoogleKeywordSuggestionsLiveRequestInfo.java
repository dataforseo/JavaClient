...
public class DataforseoLabsGoogleKeywordSuggestionsLiveRequestInfo  {
  ...
  /**
   * <em>search for the exact phrase</em><br>optional field<br>if set to <code>true</code>, the returned keywords will include the exact keyword phrase you specified, with potentially other words before or after that phrase<br>default value: <code>false</code>
   * @return exactMatch
   */
  @javax.annotation.Nullable
  ...
  /**
   * <em>results sorting rules</em><br>optional field<br>you can use the same values as in the <code>filters</code> array to sort the results<br>possible sorting types:<br><code>asc</code> - results will be sorted in the ascending order<br><code>desc</code> - results will be sorted in the descending order<br>a comma is used as a separator<br>example:<br><code>['keyword_info.competition,desc']</code><br>default rule:<br><code>['keyword_info.search_volume,desc']</code><br><strong>note that you can set no more than three sorting rules in a single request</strong><br>you should use a comma to separate several sorting rules<br>example:<br><code>['keyword_info.search_volume,desc','keyword_info.cpc,desc']</code>
   * @return orderBy
   */
  @javax.annotation.Nullable
